...
namespace Restaurante.Services
{
    public class CategoriaServices : ICategoriaServices
    {
        ...
        public async Task<ResultResponse> Editar(EditarCategoriaDTO dTO)
        {
            ...
            var currentCategory = await _unitOfWork.Categoria.Where(x => x.Id == dTO.CategoriaId).Take(1).FirstOrDefaultAsync();
        ...
        {
            var result = new ResultResponse();
            if (_repository.Exists(categoriaId))
            {
                var cat = await _repository.Where(x => x.Id == categoriaId).FirstOrDefaultAsync();
