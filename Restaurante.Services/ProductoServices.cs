    ...
    public class ProductoServices : IProductoServices
    {
        ...
        public async Task<ResultResponse> Consumir(ConsumirProductoDTO dto)
        {
            ...
            if(ids.Count() > 0)
            {
                ...
                products.AddRange(await _unitOfWork.Producto.Where(x => ids.Contains(x.Id))
        ...
        public async Task<ResultResponse> Editar(EditarProductoDTO dTO)
        {
            ...
            var currentProduct = _unitOfWork.Producto.Where(x => x.Id == dTO.ProductoId && x.DeletedAt == null).Take(1).FirstOrDefault();
        ...
        public async Task<ResultResponse> Eliminar(string productId)
        {
            ...
            if (_unitOfWork.Producto.Exists(productId))
            {
                var currentProduct = _unitOfWork.Producto.Where(x => x.Id == productId).FirstOrDefault();
                await _unitOfWork.BeginTransactionAsync();
        ...
        public async Task<ResultResponse> Restaurar(string productId)
        {
            ...
            if (await _unitOfWork.Producto.Restore(productId))
            {
                await _unitOfWork.SaveChangesAsync();
                await _unitOfWork.CommitTransactionAsync();
                result.Content = await _unitOfWork.Producto.Where(x => x.Id == productId).FirstOrDefaultAsync();
        ...
        public async Task<ResultResponse> RestaurarIngredientes(ConsumirProductoDTO dto)
        {
            ...
            ingredientes.AddRange(await _unitOfWork.Ingrediente.Where(x => ingredienteIds.Contains(x.Id)).ToListAsync());
